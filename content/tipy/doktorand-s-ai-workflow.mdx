...
excerpt: "Kompletní návod s prompty na doktorandský týden: pondělní rešeršní rutina s citacemi, knihovna článků, která odpovídá jen z vašich zdrojů, příprava cvičení z vlastních podkladů, opravování písemek proti bodovacímu klíči i rukopisu, šablony na administrativu — a kde AI musí zůstat jen poradcem."
category: "ai"
platform: "vsude"
audience: ["student"]
keys: []
...
Sledovat obor znamená jednu nudnou činnost týdně: projít, co vyšlo, a rozhodnout, co si přečtete. Hledání je mechanika, čtení ne. Právě hledání jde předat.

### Nastavení rutiny

Nejlepší forma je **plánovaná úloha** — rutina, která se spustí sama podle rozvrhu a výsledek na vás čeká v pondělí ráno. Nemusíte si vzpomenout, nemusíte otevírat databáze. U Claude jde o rutinu spouštěnou podle rozvrhu, podrobněji v tipu [rutiny nad poštou a kalendářem](/tipy/claude-rutiny-posta-kalendar).
...
Když otevíráte novou kapitolu nebo se školitel zeptá na oblast, kterou neznáte, sáhněte po jednorázové hloubkové rešerši — režimu, který sám prohledá web a vrátí zprávu s odkazy. Ptejte se v ní na totéž co v rutině a navíc na dvě věci: v čem se autoři neshodnou (u sporu obě strany) a co podle dostupných prací zůstává nezodpovězené. Ta druhá odpověď je pro doktoranda nejcennější, protože právě mezera v poznání je místo, kam se vejde disertace. Postup je rozepsaný v tipu [hloubková rešerše](/tipy/deep-research-reserse-trhu).
...
Veďte si jednu tabulku: autor, rok, název, kde je soubor uložený, k jaké části disertace se vztahuje, stav „přečteno / prolétnuto / neotevřeno" a jednou větou proč jste to zařadili. Bez ní budete za dva roky hledat, ve kterém PDF byla ta jedna definice. Rutina vám do ní každý týden přisype pár řádků — a to je celý smysl.
...
Bod 4 je nenápadný a šetří ostudu: v každém oboru koluje pár tvrzení, která všichni citují z druhé ruky a původní studie říká něco jiného. Bod 3 je zase materiál na diskusi ve vašem článku — vysvětlit cizí rozpor metodologicky je silnější než ho jen konstatovat.

### Vlastní poznámky ke článku

Čtení nedelegujete, ale výpisky jde zrychlit. Model vám ke článku připraví kostru poznámky, kterou při čtení doplníte a opravíte — a hlavně vám vypíše, co si má člověk ověřit vlastníma očima.

```text
Vezmi zdroj [název souboru] a připrav mi podklad ke čtení:
- hlavní teze autora v 5 větách
- design a vzorek
- 5 nejdůležitějších zjištění, u každého číslo stránky
- 3 citovatelné pasáže v přesném znění, s číslem stránky
- na co si mám dát při čtení pozor: kde jsou závěry silnější
  než data, kde chybí kontrolní skupina, kde je vzorek úzký
- 3 otázky, které bych autorovi položil

Vycházej výhradně z tohoto zdroje. Kde není stránka k dohledání,
napiš to místo odhadu.
```

Předposlední bod z toho dělá čtecí nástroj místo náhražky čtení. A jedna technická poznámka ke skenům: u starých nebo křivě naskenovaných PDF se OCR plete v číslicích a diakritice, u dvousloupcové sazby míchá odstavce. Než z takového zdroje převezmete číslo nebo doslovnou citaci, otevřete originální stránku a porovnejte.
...
Vrátí hodinu, kterou stačí projít a upravit. Řešení příkladů si **spočítejte sami** — zejména u kvantitativních oborů model počítá „od oka" a výsledek vypadá věrohodně. U příkladů s výpočtem je bezpečnější nechat si napsat skript, který čísla spočítá, viz [skripty pro nevývojáře](/tipy/ai-skripty-pro-nevyvojare). Bod 3 je ta část, kterou jinak získáte až po třech letech učení.
...
Bod 2 s hranicí analogie je důležitý: analogie, kterou studenti dotáhnou dál, než platí, vyrobí chybu, kterou pak odstraňujete půl semestru. Víc o vysvětlování je v tipu [vysvětlení látky s AI](/tipy/ai-vysvetleni-latky).
...
Tady je největší úspora času a zároveň nejtvrdší hranice celého návodu. Pořadí obojího není náhodné.
...
- **ověřte pravidla své univerzity** — některé fakulty mají nahrávání studentských prací do externích služeb výslovně upravené a je jednodušší se zeptat předem než vysvětlovat potom

Naskenovat sadu písemek zvládne telefon; užitečné je použít režim skenování dokumentů, který srovná perspektivu, viz [skenování do poznámek](/tipy/ios-skenovani-poznamky).
...
Vrátí klíč, který unese padesát písemek beze změn v průběhu. Bod 3 je ten, kvůli kterému se prompt vyplatí: alternativní správné řešení, na které jste nepomysleli, je nejčastější důvod reklamací.
...
Bod 3 je zpětná vazba na vaši výuku, ne na studenty. Úloha, kterou nezvládla polovina skupiny, obvykle znamená, že se něco na cvičení nepodařilo vysvětlit — a to je informace, se kterou se dá příští rok pracovat.
...
Vrátí koncepty, které projdete a odešlete pod svým jménem. Dvě věci hlídejte: aby zpětná vazba neobsahovala tvrzení, které v písemce není, a aby tón seděl vaší skupině — model má sklon k mírné patetičnosti, kterou vysokoškoláci vycítí na první přečtení.
...
Nejlepší šablony nevznikají z ničeho — vznikají z vašich vlastních odpovědí, které jste už napsali. Podrobný postup je v tipu [šablony z odeslané pošty](/tipy/sablony-z-odeslane-posty-ai).
...
Koncept si vždycky přečtěte a doplňte — model neví o tom, co jste nezapsali. A odesílá se sám, tedy vy: **AI navrhuje, člověk odesílá.**
...
- **Projekty s trvalým kontextem** — jeden projekt na předmět, který učíte, s vaším značením, klíčem a zvyklostmi; nemusíte je vypisovat pokaždé znovu, viz [projekty jako trvalý kontext](/tipy/ai-projekty-kontext).
...
- **Peníze:** nepřímo. Rok studia navíc stojí rok vaší mzdy jinde; a včas odevzdaný článek bývá podmínkou grantu nebo stipendia.
