...
excerpt: "Kompletní návod s prompty: jak diktovat, aby byl přepis použitelný, jak ho jedním promptem učesat do mailu, zápisu nebo draftu, jak zvládnout i dlouhé texty po blocích a jak z chůze a cest udělat psací čas."
category: "ai"
platform: "vsude"
audience: []
keys: []
...
Návod je stavěný tak, abyste ho mohli aplikovat po částech. První dvě fáze jsou nastavení a technika mluvení, třetí a čtvrtá jsou jádro — prompty na učesání a prompty podle účelu výstupu. Pátá fáze řeší dlouhé texty, šestá diktování za chůze a v autě, sedmá to, aby výsledek zněl jako vy a ne jako robot. Každá fáze má aspoň jeden kompletní prompt k okopírování, stačí doplnit hranaté závorky. Nad vším platí jedno pravidlo: **AI navrhuje, člověk schvaluje** — odeslat mail, uložit zápis nebo publikovat text smí až vy, po přečtení.
...
U počítače pak proběhnou tři kroky. Nejdřív úklid: prompt, který odstraní vycpávky a opakování, ale nic obsahového nezahodí. Pak strukturování podle účelu: „tohle je podklad pro vedení, chci shrnutí na půl strany plus tři rizika s návrhem opatření". Nakonec kontrola — Petr si výstup přečte a opraví dvě místa, kde model z jeho „to asi nestihneme" udělal „termín je ohrožen", což je jiná míra jistoty. Celkem dvacet minut, z toho dvanáct za chůze. Ušetřená hodina není hlavní zisk; hlavní zisk je, že podklad vzniká z toho, co ho napadlo za tepla, ne z toho, co si po hodině sezení vzpomněl.

## Fáze 1: nastavení, které zabere pět minut

Diktování selhává nejčastěji na tom, že si ho člověk musí pokaždé někde zapínat, hledat, kam text padá, a po třech pokusech to vzdá. Cílem téhle fáze je, abyste mohli začít mluvit do dvou vteřin od chvíle, kdy vás něco napadne.

### Kde diktování zapnout

Na **Windows** je zabudované pod <kbd>Win</kbd> + <kbd>H</kbd>. Funguje v jakémkoli textovém poli — ve Wordu, v mailu, v chatu s AI. Detailně je popsané v tipu [diktování na Windows](/tipy/win-h-diktovani).

Na **Macu** se diktování zapíná klávesovou zkratkou, kterou si nastavíte v systémových nastaveních (typicky dvojí stisk vybrané klávesy); podrobnosti jsou v tipu [diktování na Macu](/tipy/mac-diktovani). Výhoda Macu je, že u novějších systémů běží rozpoznávání přímo v zařízení, takže funguje i bez internetu.

Na **mobilu** máte mikrofon přímo v klávesnici — na Androidu i na iPhonu. Je to nejrychlejší cesta k zachycení nápadu, protože telefon máte u sebe vždycky. Praktický postup je v tipu [diktování textu](/tipy/diktovani-textu) a pro rychlé zprávy v [diktování zpráv na mobilu](/tipy/mobil-diktovani-zprav).
...
**Google Dokumenty** mají vlastní hlasové psaní, které je pro dlouhé texty klidnější než systémový diktát, protože píše rovnou do dokumentu, který se sám ukládá — viz [hlasové psaní v Dokumentech](/tipy/docs-hlasove-psani).

### Kam přepis padá

Zvolte jedno místo a držte se ho. Nejlépe funguje jeden „sběrný" zápisník — poznámková aplikace, kterou máte na mobilu i na počítači a která se synchronizuje sama. Do něj padá všechno syrové, ideálně s datem a dvouslovným označením na začátku („porada Nováková", „nápad na newsletter"). Zpracované věci ze zápisníku odcházejí do mailu, do zápisu nebo do dokumentu; zápisník zůstává skládkou, ne archivem.

Pokud už používáte systém rychlého zachycování nápadů, napojte diktování na něj — princip je popsaný v tipu [hlasový capture](/tipy/hlasovy-capture). Podstatné je, aby cesta od nápadu k přepisu neměla víc než dva kroky.
...
## Fáze 2: jak mluvit, aby z toho šel udělat text
...
Systémové diktáty umí diktovanou interpunkci („čárka", „tečka", „nový odstavec"). Pokud píšete krátkou zprávu rovnou do pole a nechcete ji dál zpracovávat, vyplatí se ji diktovat. Pokud přepis stejně půjde do AI, **interpunkci neřešte** — přerušuje tok řeči a AI ji doplní lépe, než vy ji nadiktujete. Jediná výjimka je odstavcování u dlouhých bloků; hlasová značka „nová sekce" tam poslouží líp než „nový odstavec".
...
## Fáze 3: prompty podle účelu výstupu
...
Vrátí zápis, do kterého stačí doplnit chybějící jména a termíny. Systematičtější postup pro pravidelné porady je v tipu [zápis z porady](/tipy/ai-zapis-z-porady); pokud schůzky rovnou nahráváte, navazuje [automatické zápisy schůzek](/tipy/ai-zapisy-schuzek-automaticky).
...
Vrátí zadání plus seznam otázek, který je většinou cennější než zadání samo — ukáže, co jste považovali za samozřejmé. Souvisí s tipem [delegování s kontextem](/tipy/manazer-delegovani-kontext).

## Fáze 4: dlouhé texty po blocích
...
Vrátí souvislý text plus dva seznamy — co odstranil jako duplicitu a co sjednotil. Oba si projděte; občas vypadne verze, která byla lepší, a je to jedna oprava na půl minuty.
...
## Fáze 5: chůze, auto, fronta — kde se diktuje nejlíp

Největší zisk z diktování není rychlost psaní. Je to čas, který dosud propadal.

### Chůze

Chůze je pro mluvené přemýšlení mimořádně dobrá: tempo řeči se srovná, myšlenky se řadí za sebou a nemáte před sebou obrazovku, která by vás rušila. Dvanáct minut chůze vydá zhruba na 1500 slov surového materiálu, což jsou po učesání dvě až tři normostrany.

Praktické zásady: sluchátka s mikrofonem, telefon v kapse, aplikace spuštěná před odchodem. Nekoukejte, jestli přepis vychází správně — když se ohlížíte po textu, přestanete mluvit. Kontrola přijde u stolu.

### Auto

V autě platí jediné pravidlo: **řízení má přednost před nápadem.** Diktujte přes hands-free, spusťte nahrávání hlasovým povelem před vyjetím a mluvte v krátkých blocích s pauzami — v hustém provozu prostě přestanete mluvit a pokračujete, až se to uvolní. Nekontrolujte displej, nespravujte přeřeky, neopravujte přepis. Auto je na zachycení, ne na zpracování.

Dobrý zvyk je začít každou nahrávku větou „Nahrávám v autě, bude to útržkovité" — vy si tím nastavíte očekávání a v promptu na to pak můžete odkázat.

### Fronta, čekání, tři minuty mezi schůzkami

Krátká okna jsou ideální na jednu věc: vyprázdnit hlavu. Ne psát text, ale zbavit se toho, co v hlavě cirkuluje. Nadiktujte třicet vteřin „co mi teď leží v hlavě" a nechte to spadnout do zápisníku; večer z toho udělá dávkové zpracování úkoly a poznámky.
...
Vrátí přehled celého dne za jeden průchod. Pokud tohle děláte pravidelně, dá se to zautomatizovat: v Claude jde nastavit plánovaná rutina, která běží každý večer nad složkou s poznámkami — princip je popsaný v tipu [rutiny pro poštu a kalendář](/tipy/claude-rutiny-posta-kalendar).
...
## Fáze 6: aby výsledek zněl jako vy
...
Vrátí popis stylu, který si uložte a přikládejte ke každému promptu na učesání — nebo lépe, vložte ho do vlastních instrukcí. Postup je v tipu [vlastní instrukce](/tipy/ai-vlastni-instrukce), pro delší práci nad jedním typem obsahu se hodí [projekty a trvalý kontext](/tipy/ai-projekty-kontext).

### Uložení promptů, aby se nehledaly

Pokud budete diktovat denně, potřebujete tři až pět promptů po ruce, ne v historii chatu. Nejjednodušší je textový soubor nebo poznámka se sekcemi „úklid", „mail", „zápis", „úkoly", „draft". Systematičtější řešení popisuje [knihovna promptů](/tipy/ai-knihovna-promptu).

### Kontrola před odesláním

Poslední krok, který se nesmí vynechat: text si přečtěte. Ne prolétnout, přečíst. U mailů a zápisů, které jdou ven, se vyplatí přečíst je nahlas — je to nejrychlejší detektor vět, které nezní jako vy, a zároveň odhalí místa, kde model zvýšil jistotu tvrzení.

A platí zásada, která se táhne celým webem: **odesílá člověk.** Ať máte workflow jakkoli vyladěné, tlačítko odeslat mačkáte vy, po přečtení. Automatické odesílání mailů z diktátu je nejrychlejší cesta k tomu, aby klientovi odešla vaše poznámka pro sebe.

### Citlivé údaje

Do diktátu se v proudu řeči snadno dostane jméno klienta, číslo smlouvy nebo částka. Než přepis vložíte do AI, projděte ho a konkrétní identifikátory nahraďte obecnějším popisem („klient A", „zakázka v řádu statisíců") — na uspořádání textu to nemá vliv. Pokud pracujete s obsahem, který je opravdu citlivý, patří jen do placeného účtu se smluvní ochranou dat, ne do volně dostupného chatu.
