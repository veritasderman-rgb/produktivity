...
excerpt: "Kompletní návod s prompty: kde knihovnu vést, jak vypadá dobrý záznam, jak prompty verzovat a zlepšovat po každém použití, jak je sdílet v týmu — a deset promptů, které má mít v knihovně každý."
category: "ai"
platform: "vsude"
audience: []
keys: []
...
**Knihovna promptů není sbírka textů, je to zaznamenané know-how.** V každém osvědčeném promptu je uložené, jak vaše firma dělá zápisy, co musí obsahovat nabídka, jakým tónem se odpovídá klientům a co se nikdy neposílá bez kontroly. Tohle vědomí máte dnes v hlavě a mizí s vámi na dovolené i při odchodu z firmy. Zapsané v knihovně se dá předat, zlepšovat a měřit.

Tenhle návod vede od prázdné stránky k funkční knihovně, kterou používáte denně a která roste sama od sebe. Projdete výběrem úložiště, strukturou záznamu, verzováním, sdílením v týmu a deseti prompty, které se vyplatí mít jako první. Číst ho najednou nemusíte — fáze 1 a 2 zaberou večer, zbytek přidáte za pochodu.
...
Nejčastější důvod, proč knihovny promptů umřou, není lenost. Je to zahlcení. Když do ní ukládáte všechno, po půl roce v ní máte sto padesát záznamů, nic nenajdete a je rychlejší napsat prompt znovu. Knihovna má hodnotu jen tehdy, když je v ní **málo věcí a všechny fungují**.

### Pravidlo tří použití

Ukládejte prompt až tehdy, když ho **použijete potřetí** — nebo když víte, že ho potřetí použijete. První použití je experiment, druhé náhoda, třetí je vzorec. Do té doby ho nechte být v historii konverzací.
...
Většina promptů, které si budete ukládat, vznikla v chatu jako konkrétní zadání s konkrétními daty. Než je uložíte, musíte je „vysušit" — nahradit specifika placeholdery a doplnit to, co jste v chatu doříkali až v druhé zprávě. Tohle je práce, kterou AI udělá líp a rychleji než vy:
...
Vrátí šablonu, kterou rovnou vložíte do knihovny, a seznam placeholderů, ze kterého uděláte checklist při použití. Pozor na bod 2 — model má sklon dopisovat požadavky, které nechcete (typicky „přidej na konec shrnutí" nebo „použij emoji"). Šablonu si přečtěte řádek po řádku a co tam nepatří, smažte; jinak si do knihovny uložíte cizí zvyky.

## Fáze 2: kde knihovnu vést

Neexistuje jediné správné místo. Existují tři, každé řeší jinou situaci, a většina lidí nakonec používá kombinaci: rychlé prompty ve zkratkách, celou knihovnu v databázi, a nejpoužívanější sady jako Projekty.
...
Pro prompty je to nejrychlejší možná cesta: napíšete `;zapis` a rozbalí se celý prompt na zápis z porady. Žádné přepínání okna, žádné hledání. Konvence, která se osvědčila: prefix `;` nebo `qq` (znaky, které v běžném textu nikdy nenapíšete) plus krátké slovo — `;zapis`, `;mail`, `;shrn`, `;kontrola`.
...
Minimální sada sloupců: **Název** (sloveso plus předmět — „Připravit zápis z porady", ne „Zápisy"), **Kategorie** (výběr z pevného seznamu, ne volný text), **Kdy použít** (jedna věta), **Verze** (číslo), **Naposledy použito** (datum), **Kdo vlastní** (u týmové knihovny) a **Stav** (aktivní / k revizi / vyřazeno). Samotný text promptu patří do těla stránky, ne do buňky — v buňce se špatně čte a špatně kopíruje.
...
Funguje i jeden markdownový soubor v cloudovém úložišti: otevře se všude, dá se v něm hledat a nikdy se nerozbije. Nad padesát promptů se v něm ale ztratíte. Když tuhle cestu zvolíte, držte striktní strukturu — nadpis druhé úrovně kategorie, nadpis třetí úrovně název promptu, pak metadata a blok s textem.
...
Vypadá to na hodně psaní, ale vyplnění zabere tři minuty a děláte to jednou. Pokud máte pocit, že je to na váš případ moc, škrtněte NÁSTROJ a HISTORII; UKÁZKA VÝSTUPU a NA CO POZOR jsou to, co dělá rozdíl mezi knihovnou a hromadou textu.
...
Tady je rozdíl mezi sbírkou a knihovnou. Sbírka je statická — uložíte prompt a používáte ho, dokud nepřestane fungovat. Knihovna se učí: každé použití je zdroj informace o tom, co v zadání chybělo.
...
Tenhle jediný návyk udělá s kvalitou knihovny víc než všechno ostatní v tomhle návodu. Po deseti použitích je prompt vyladěný natolik, že opravy skoro nepotřebuje — a přesně tomuhle bodu se říká, že prompt „drží".
...
Prompt, který funguje na hezkém vstupu, se láme na tom ošklivém. Zápis z porady, kde nikdo nic neuzavřel. Reklamace bez jasného požadavku. Report za měsíc, kdy nebyla žádná data. Než prompt označíte za hotový, nechte si najít, kde spadne:
...
Vrátí seznam slabin, ze kterých obvykle dvě až tři stojí za doplnění. Bod 3 je tam schválně jako brzda — bez něj vám model navrhne patnáct pravidel a prompt se zdvojnásobí. Nejčastější užitečný nález bývá ošetření prázdného vstupu: instrukce ve stylu „když v podkladu chybí termín nebo odpovědná osoba, napiš NEUVEDENO, nedomýšlej si" ušetří spoustu tichých chyb.
...
Bod 4 bývá překvapivě přesný — z toho, co v knihovně je, jde odhadnout, co ve vaší práci existuje a nemáte to pokryté. Nálezy ale berte jako návrhy: model nevidí, jak často co používáte, takže „nepoužívané" prompty ověřte proti vlastní paměti, ne jen proti datu.

## Fáze 5: sdílení v týmu

Osobní knihovna šetří vám. Týmová knihovna mění to, jak rychle se v týmu zaučuje a jak konzistentní jsou výstupy ven. Je to zároveň místo, kde se nejčastěji dělá chyba: sdílená složka, do které kdokoli cokoli hodí, se během měsíce změní v smetiště.
...
Každý sdílený prompt má **vlastníka** — konkrétního člověka, ne „tým". Vlastník ho aktualizuje a jemu se hlásí, že přestal fungovat. Bez vlastníka nikdo neopraví nic.
...
Než knihovnu otevřete širšímu okruhu, projděte ji jedním průchodem na citlivá data. Lidé do promptů zapisují konkrétní věci, aniž by o tom přemýšleli — jméno klienta v příkladu, částku v ukázce výstupu, interní přezdívku projektu:
...
Vrátí seznam, který projdete ručně. Dvě věci hlídejte: model spolehlivě najde jména a čísla, ale hůř pozná **interní kontext**, který je citlivý jen u vás (název nevydaného produktu, jméno projektu, se kterým se ještě nechlubíte). A ukázky výstupu jsou opravdu nejrizikovější místo — proto jsou v promptu zmíněné zvlášť.

Stejný průchod pusťte na cokoli, co posíláte mimo firmu. Prompt na strukturu zápisu je neškodný a klidně ho sdílejte veřejně; prompt na cenovou nabídku, ze kterého je vidět vaše logika marží, ven nepatří.
...
1. **Zápis z porady s úkoly** — z poznámek nebo přepisu udělá strukturovaný zápis, kde jsou zvlášť rozhodnutí a zvlášť úkoly s termíny.
2. **První návrh odpovědi na e-mail** — z došlé zprávy a vaší poznámky o záměru udělá draft, který ještě upravíte.
3. **Shrnutí dlouhého dokumentu s riziky** — ze smlouvy, nabídky nebo zprávy vytáhne podstatu a hlavně to, co by mohlo být problém.
4. **Kontrola textu před odesláním** — projde váš hotový text a označí, co je nejednoznačné, co chybí a co může být špatně pochopeno.
5. **Příprava na jednání** — z podkladů udělá strukturu schůzky, otázky a předvídané námitky protistrany.
6. **Rozpad velkého úkolu na kroky** — z mlhavého zadání udělá seznam konkrétních kroků s odhadem, co je první.
7. **Převod poznámek na strukturovaný dokument** — z hromady odrážek udělá text s nadpisy a logickým sledem.
8. **Vysvětlení něčeho, čemu nerozumíte** — technický text, právní odstavec, cizí obor, vysvětleno na vaší úrovni.
9. **Oponentura vlastního nápadu** — nechá si najít slabiny v tom, co chcete navrhnout, dřív než je najde někdo jiný.
10. **Týdenní ohlédnutí** — z kalendáře a poznámek udělá přehled toho, co se udělalo a co zůstalo viset.
...
Zákaz přepisování je podstatný: kdybyste nechali model text opravit, dostanete jiný text, ale ne lepší dovednost. Nálezy z bodu 1 bývají překvapivé — nejednoznačnost ve vlastním textu autor nevidí prakticky nikdy. Prompt funguje i na zprávy do chatu, jen si u krátkých textů vyžádejte jen body 1 až 3.
