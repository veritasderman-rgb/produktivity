...
excerpt: "A complete guide with prompts: four summary types by purpose, extracting risks from contracts, comparing two versions, and checking nothing vital got lost."
category: "ai"
platform: "vsude"
...
updated: "2026-08-12"
tldr: >-
  A summary isn't a shortened document — it's an answer to the question you're reading the document for, which is why the summary type (executive, decision, research, comparison) goes into the first line of the prompt. The guide also covers very long documents processed in parts, extracting obligations and risks from contracts, comparing two versions, and the most valuable step: a reverse check asking what got lost in the summary. A summary is a tool for reading smarter, not for not reading.
faq:
  - q: >-
      Why doesn't “summarize this document” work?
    a: >-
      It's the weakest prompt you can write: without a purpose, a reader, and a structure, you get a generic paragraph that doesn't tell you what to do. An excerpt for a meeting looks different from a decision brief — when you don't state the purpose, the model picks one for you, and it picks the most generic one available.
  - q: >-
      How do I know the model read the whole document?
    a: >-
      First have it describe what it sees in the file — page count, chapter names, the first and last sentence. If it lists five chapters and your table of contents has eight, it's working with only part of the file. A quick test for long files: “quote the first paragraph of chapter 6 verbatim” — if it can't, it isn't seeing the whole document and it's time to process it in parts.
  - q: >-
      Can I trust the page references?
    a: >-
      Not without checking — a page reference looks like proof, but with long documents page numbers often get guessed. Spot-check two or three. And the rule for numbers: every number you say in a meeting or write into a document that goes out, you must be able to point to in the original.
  - q: >-
      What gets lost in summaries most often?
    a: >-
      Conditions and exceptions — and summaries also systematically strengthen conclusions: “suggests” becomes “proves,” a conditional claim becomes unconditional. That's what the reverse check is for: give the model the summary and the original and have it hunt for what's missing or stated more strongly — ideally with a different model than the one that wrote the summary.
  - q: >-
      Can AI review a contract instead of a lawyer?
    a: >-
      No — AI is not a legal advisor. What it does well: pulls obligations with deadlines into a table, flags unbalanced and unusual clauses with citations, and lists what's missing from the contract. Assessing legal compliance belongs to someone qualified; the extraction saves you time with them, because you show up with specific articles, not a vague feeling.
  - q: >-
      Can I upload a document with personal data?
    a: >-
      Sensitive data belongs only in a paid account with contractual data protection — and even there, only what the analysis actually needs. Strip ID numbers, addresses, and third-party contacts beforehand, replace names with roles. For documents under an NDA, first check whether you're allowed to process them with an external tool at all.
---
