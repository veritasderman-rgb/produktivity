...
updated: "2026-08-12"
tldr: >-
  You don't need to memorize function syntax: describe the result you want in plain words and AI returns a formula plus an explanation of why it works. What's left for you is the part a machine can't do — deciding what should actually be calculated and verifying that the number is correct. The guide covers phrasing formula requests, taking apart inherited formulas, debugging error messages, workbook structure, pivot tables, and macros.
faq:
  - q: >-
      What do I have to put in the request so the formula works the first time?
    a: >-
      Five things: the program and version (older Excel doesn't have XLOOKUP), where the data is (exact columns and the starting row), a sample of three to five rows, what exactly should come out, and the formula's language — a localized version of Excel has localized function names and a semicolon as the argument separator. Without that you get the English version and have to translate it by hand.
  - q: >-
      The formula returns a number — can I trust it?
    a: >-
      Not automatically: a formula that returns a number isn't yet a formula that returns the right number. Verify it on a row you can compute by hand, and check a control total — the sum of the parts must equal the whole. The most common silent error of all is a range shifted by relative references after copying the formula down.
  - q: >-
      What should I do with a #N/A error?
    a: >-
      Nine times out of ten the cause is an extra space, different capitalization, a number stored as text, or a lookup pointed at the wrong column. Above all, don't hide the error with IFERROR until you know why it's happening — otherwise you sweep the problem out of sight and data quietly goes missing from your totals.
  - q: >-
      Can I upload a whole workbook with business figures into a chat?
    a: >-
      You don't need to and shouldn't: deriving a formula only needs the structure and sample data. Replace the real values, and handle sensitive business data only in a paid account with contractual data protection.
  - q: >-
      When should I use a formula, a macro, or a script?
    a: >-
      A formula calculates over data in the workbook and recalculates itself — that's most tasks. A macro (VBA) or Apps Script fits repeated manipulation of the workbook itself, like merging five files or monthly archiving. An external Python script fits large data volumes and combining multiple sources. Always test a macro on a copy of the data, never on the original.
  - q: >-
      Can I keep a formula I don't understand?
    a: >-
      No formula you can't explain in one sentence gets into a workbook you hand to someone else. Have it taken apart from the inside out — including the circumstances under which it returns a wrong value without showing an error. If you can't explain what it does, you also won't notice the moment it starts doing something else.
---
