...
updated: "2026-08-12"
tldr: >-
  Claude can build a working mini-app right inside the conversation — a calculator, quiz, or prototype — that you share with a link, no installation and no developer. The guide runs from a brief with a control example through one-change-at-a-time iteration and edge-case testing to sharing, maintenance, and the boundary where an artifact stops being enough. Two rules above everything: the artifact computes, the human decides — and sensitive data doesn't belong in it.
faq:
  - q: "What can an artifact do well, and what shouldn't I expect?"
    a: >-
      It handles calculators, interactive tools, quizzes, trackers, prototypes, and visualizations — anything that's “formula plus rules plus exceptions.” But it has no shared memory between people: what one person enters, another won't see, and it's usually gone when the window closes. It doesn't reach into your company systems and has no login or audit trail.
  - q: "How do I know the calculator computes correctly?"
    a: >-
      Put a control example in the brief with a result you computed by hand — without it you have no way to tell the tool is wrong, because a formula error is silent. Before sharing, also have it list the rules it computes by, and especially the values the model filled in on its own — that list tends to be longer than you expect.
  - q: "How should I phrase edits so nothing breaks?"
    a: >-
      One change, one sentence — after a batch of ten edits you won't know which one broke things. Describe behavior, not implementation, say what must not change, and have the control example recomputed after every batch. If a version goes wrong, the panel keeps older versions and you can roll back.
  - q: "Can I put client data or salaries into an artifact?"
    a: >-
      No — work with rates, rules, and formulas, not a client database, salaries, or health data. If you must work with sensitive data, only on a paid or business account with contractual data protection, and anonymize everything that can be anonymized.
  - q: "Can the tool decide or send things on its own?"
    a: >-
      No — an artifact may compute and suggest; the decision stays with a human. A traffic light, recommendation, or score is input for a person; the moment the tool sends, approves, or grades something, you've crossed the line into automation, which deserves a different approach.
  - q: "When does an artifact stop being enough?"
    a: >-
      Five signals: data needs to persist and be shared by multiple people, personal data is involved, you need an audit trail, the tool must talk to your systems, or it has become critical. The artifact then isn't wasted work — it's the best possible spec for a developer: a working model tested by real users.
---
