...
excerpt: "A guide with prompts: when a Claude routine is enough and when you need n8n, Zapier, or Make, three full scenarios step by step, and error handling."
category: "ai"
platform: "vsude"
...
updated: "2026-08-12"
tldr: >-
  Your first no-code AI automation: AI isn't the whole automation, but one step in
  the middle — something triggers the scenario, the model extracts or writes
  structured output, and the result lands in a spreadsheet, a CRM, or a Drafts
  folder. The guide decides when a Claude routine is enough and when you need Zapier,
  Make, or n8n, walks through three complete scenarios, and covers error handling.
  The automation may prepare anything; the final click stays yours.
faq:
  - q: "When is a Claude routine enough, and when do I need an automation platform?"
    a: >-
      A routine is enough when the trigger is time, the data is reachable through a
      connector, the brief changes often, and the output is text for a human. You
      need a platform when the trigger is an event in another system (a form, an
      invoice, a payment), more than two apps are in the chain, or you need a
      webhook and reliable runs over volume. Resolve borderline cases with the
      simpler option.
  - q: "Why is a prompt in an AI step written differently than in chat?"
    a: >-
      Because you won't see it fail, you can't ask follow-ups, and the output is
      read by a machine, not a person. Five rules apply: one step, one task; a fixed
      output format (JSON with enumerated keys); an explicit ban on guessing with a
      “not stated” value; closed category lists instead of free-form ones; and a
      sample input and output right in the prompt.
  - q: "May the automation send emails or pay on its own?"
    a: >-
      No — AI proposes, a human approves. Three rules without exception: a draft
      instead of a sent email, a task labeled “needs approval” instead of a task in
      the live project, a new row instead of overwriting an existing one. A draft
      costs you ten seconds of reading; one unfortunate sent email costs a client.
  - q: "How do I find out a scenario has silently broken?"
    a: >-
      A silent scenario looks the same as a working one. Every scenario needs a
      failure path — a separate table where everything that failed lands, plus one
      daily summary alert. And run an artificial test once a month: it's the only
      way to tell “nothing happened” from “the scenario hasn't run in a week.”
  - q: "What if the model returns valid but nonsensical output?"
    a: >-
      That's the most insidious failure, because the scenario completes as
      successful. The defense is a sanity check before writing: a budget outside the
      expected range, a date in the past, a category outside the list, or an empty
      required field should route to a manual queue, not into the CRM.
  - q: "When is it better not to automate at all?"
    a: >-
      When you do it less than three times a week, when the process is unstable,
      when the cost of an error is high and the error is invisible, when it's about
      a relationship rather than data transfer (a complaint, condolences, price
      negotiation), when there are more exceptions than rules — and when the
      scenario would need rights you wouldn't give an intern.
---
