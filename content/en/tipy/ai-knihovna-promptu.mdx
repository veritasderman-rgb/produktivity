...
excerpt: "A complete guide with prompts: where to keep a library, how to version and improve prompts, how to share them in a team — and ten every library needs."
category: "ai"
platform: "vsude"
...
tested: "2026-08-15"
tldr: >-
  A prompt library isn't a collection of text, it's recorded know-how: every proven prompt has baked into it how your company writes meeting notes, proposals, and client replies. The guide walks from choosing where to store it through structuring an entry (with a sample output and a “watch out for” note) to versioning and team sharing. The key rules: save a prompt only after its third use, and fold every correction you type after an output back into the prompt.
faq:
  - q: >-
      When is a prompt worth saving to the library?
    a: >-
      Only once you use it a third time — the first use is an experiment, the second a coincidence, the third a pattern. Don't start a library by inventing thirty prompts in advance: invented prompts never get used, because they didn't come from a real need. After a month of saving real work you'll have ten to fifteen entries, and that's a library, not a wish list.
  - q: >-
      What must an entry contain?
    a: >-
      Besides the prompt text with descriptive placeholders, above all a sample of a good output — it tells you what to expect from the prompt and lets you notice when results get worse. Then a one-sentence “when to use,” a version with a change note, and a “watch out for” section with typical failures.
  - q: >-
      Can I put client names and amounts into prompts?
    a: >-
      No — a prompt is a template, and where specific data would go there should be placeholders in square brackets. That's what makes the library shareable and keeps a prompt from aging out with a client. And the general rule applies: sensitive data belongs only on a paid account with contractual data protection, anonymized even there.
  - q: >-
      How does the library get better?
    a: >-
      Through the “thirty seconds after the output” habit: whenever you type a correction after an output (“but shorter,” “deadlines are missing”), that correction belongs in the prompt — open the entry, add the line, bump the version. After ten uses a prompt is tuned enough that it barely needs corrections.
  - q: >-
      Do sentences like “be concise” belong in the library too?
    a: >-
      No — standing preferences belong in custom instructions, which apply automatically to every conversation, and unchanging context (tone of voice, product description) belongs in a Project. What stays in the prompt is only what changes case by case.
  - q: >-
      How do I share the library in a team without it becoming a dump?
    a: >-
      Two layers: a shared layer with approved prompts and a personal sandbox for experiments. A prompt moves to the shared layer only once the author has used it at least three times and someone else confirmed it works for them too. Every shared prompt has a specific owner — without an owner, nobody fixes anything. Before sharing, sweep the library for sensitive data, especially in sample outputs.
---
