...
Worth mentioning among the rest: Projects (persistent context where you drop in source material and instructions once, without repeating them), Artifacts (mini-apps built right inside a conversation — a calculator, a dashboard, a simple tool), subagents in Claude Code, deep research with citations, and the Claude extension for Chrome, where an agent drives the browser under human supervision. Since spring 2026 the lineup also includes add-ins for Word, Excel, and PowerPoint (Outlook in beta) — Claude runs as a sidebar directly inside Microsoft 365, not just as a connector, sharing context across the apps.

**Where it falls short.** The surrounding ecosystem is the smallest of the three. No image or video generation, no dedicated search engine. The mobile app is usable but spare compared to the competition — voice mode isn't as fluid, and it's missing small conveniences people are used to from other apps. And last: anything interesting — custom connectors, routines, Cowork — requires a paid plan. The free tier is more for trying it out than for actual work.
...
NotebookLM, from the same stable — renamed Gemini Notebook in mid-2026, though it's functionally still the same tool — deserves a special mention. It isn't a chat competitor — it's a different kind of tool: **it answers exclusively from the sources in the notebook, and that's its main strength.** For studying, legal research, or working with internal documentation, that property is worth more than any amount of creativity.
...
| Integration into document editors | yes, add-in for Word, Excel, PowerPoint | no, only via connectors | yes, sidebar |
