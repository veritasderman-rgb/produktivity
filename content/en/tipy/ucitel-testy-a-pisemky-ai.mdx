...
excerpt: "A complete guide with prompts: a test built only from covered material, equivalent A/B/C versions, grading scanned papers. The teacher assigns the grade."
category: "ai"
platform: "vsude"
audience: ["ucitel"]
keys: []
...
updated: "2026-08-12"
tldr: >-
  A test costs a teacher four to six hours, of which maybe one is pedagogy — the
  rest is mechanics AI can handle: questions built only from your materials,
  equivalent A/B/C versions, a scoring key written in advance, proposed points
  from scanned papers with rationale, and a class-wide error analysis. What AI
  can't do is one thing: the teacher assigns the grade — a proposed score is
  input to a decision, not the decision.
faq:
  - q: "Can I just generate a test by naming the topic?"
    a: >-
      No — the model doesn't know your sequence or your textbook, and it'll
      produce a textbook-correct test on material you may not have covered.
      A test may contain only what's in your own materials: class notes,
      textbook pages, and homework. The easiest habit is photographing the
      board after every lesson into the subject folder.
  - q: "Is swapping the numbers enough for version B?"
    a: >-
      No — students spot it in a minute, and different numbers often mean
      different difficulty (an equation with a whole-number result is easier
      than the same one with a fraction). Equivalent versions share the same
      structure question by question, use different contexts, and come with
      a difficulty comparison table. And solve every version yourself before
      handing it out.
  - q: "Can AI grade the tests and assign grades?"
    a: >-
      Propose points yes, assign grades no — a model reading handwriting will
      sometimes read a 6 as an 8, miss a minus sign, or not notice the answer
      continues on the back. Require a rationale for every question, hand-grade
      at least five papers for comparison, and always judge borderline papers
      yourself. A grade is a decision about a child, and decisions about
      children don't get delegated.
  - q: "Can I upload scans with students' names on them?"
    a: >-
      No — student work is personal data. Pre-write numbers on the papers
      before handing them out and keep the number-to-name list to yourself;
      you then scan papers with no names. Work in a school-managed paid
      account with a data-protection agreement, and delete the conversation
      and files once grading is done.
  - q: "What's the point of a scoring key written in advance?"
    a: >-
      Inconsistent grading doesn't come from unfairness but from fatigue — the
      first papers get graded generously, the twentieth strictly. The key
      spells out points for method, result, and partially correct answers, and
      handles the error that carries through. Without it you have nothing to
      compare the AI's proposal against.
  - q: "What do I do with the test after entering the grades?"
    a: >-
      A class-wide error analysis — the only part of a test that improves
      teaching, and the cheapest step of the guide, since the materials
      already exist. The difference between “they make fraction errors” and
      “half the class cancels across a sum because they don't understand what
      canceling means” is the difference between useless and useful review.
---
