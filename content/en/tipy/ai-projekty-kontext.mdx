...
tested: "2026-08-27"
tldr: >-
  A project is a space where persistent instructions, uploaded materials, and every conversation on one topic live together — context stops being something you write and becomes something you have. The guide moves from a materials audit through style sheets and terminology glossaries to instructions with a “missing material” rule that stops the model from inventing facts. The key decision rule: anything you're about to say a third time belongs one level up.
faq:
  - q: >-
      When is a project worth it, and when is it pointless?
    a: >-
      It pays off when two conditions hold at once: you'll keep coming back to the topic (at least weekly, for months) and there's context that doesn't change — documents, rules, style. For a one-off research task or a single email, open an ordinary conversation and let it die.
  - q: >-
      What goes into the project versus global instructions?
    a: >-
      Global instructions = me: language, answer length, profession, what I never want — true regardless of topic. Project instructions = this work: the model's role, domain rules, format, sources. The rule of thumb: am I about to say this a third time? Then it belongs one level up. Don't copy global rules into projects — duplication breeds conflict.
  - q: >-
      Does the model answer only from the uploaded materials?
    a: >-
      No — it blends them with general knowledge, and the boundary is invisible. That's why instructions need a “sources of truth” rule: take facts exclusively from the materials, and when the answer isn't there, write MISSING MATERIAL instead of guessing. A tool that genuinely answers only from uploaded sources is NotebookLM (Google renamed it to Gemini Notebook in July 2026; the underlying principle is unchanged).
  - q: >-
      Can I upload contracts and personal data into a project?
    a: >-
      Only what your client contract allows. Sensitive material — personal data, salaries, non-public results — belongs only in a paid account with contractual data protection, and even there anonymized: replace names with roles, strip contacts and contract numbers. Check the anonymized version with your own eyes; models miss data in tables and footers.
  - q: >-
      Does uploading as many files as possible help?
    a: >-
      The opposite — more materials don't mean better answers. Past a certain volume, quality drops because the relevant passage gets lost in the noise, and near the size limit the project quietly works with only part of the material. Choose files the way you'd brief a new colleague on day one, not the way you'd build an archive.
  - q: >-
      When should I start a new project instead of adding to the existing one?
    a: >-
      When the contexts contradict each other (two clients with mutually exclusive rules — the strongest reason), when instructions would need conditionals like “different for client A,” or when materials overlap by name. But don't split by time period or individual tasks — that produces dozens of dead spaces.
---
...
- **Custom GPTs** — for one narrow purpose shared with other people (say, reviewing contracts against company rules), not for a broad, ongoing agenda. Since August 2026, only business accounts (Business/Enterprise/Edu) can create new custom GPTs — on a personal plan (Free/Plus/Pro) you can still use and edit ones that already exist.
- **NotebookLM (now Gemini Notebook)** — when you need certainty that an answer comes exclusively from uploaded sources, with a reference to the passage; a complement to projects, not a replacement.
