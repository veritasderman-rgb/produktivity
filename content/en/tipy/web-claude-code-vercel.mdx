...
excerpt: "A guide with prompts: a website without hiring a developer — from scaffolding through git as a safety net to a domain and Vercel. Proven on this site."
category: "ai"
platform: "vsude"
...
updated: "2026-08-13"
tldr: >-
  Between a template builder and an expensive developer there's a third path:
  a custom-built website you run yourself, with an agent instead of a dev team.
  The guide covers deciding where content lives (files in the repository instead
  of a database), git as a safety net, the domain and Vercel deployment, and the
  ten-minute cycle of an ordinary change — proven on the very site you're
  reading. It's not a website without work, it's a website without someone
  else's developer: the agent builds, git protects, a human ships.
faq:
  - q: "Do I need to know how to code?"
    a: >-
      No — you need to read and decide. When the agent says “build failed,
      missing export”, you don't need to know what an export is; you need to
      read the sentence and reply “fix it”. The work is elsewhere: knowing what
      you want, answering the agent's questions, and writing the content.
      Anyone expecting to say “make me a website” and go get coffee gets
      something that looks like a website until they try to use it.
  - q: "Why content in files instead of a database or CMS?"
    a: >-
      For one to three authors and text content, MDX files in the repository
      win: versioning comes free, the agent sees all articles at once and can
      fix them in bulk, and files let you cheaply build things that would be
      a whole project in a database. Choose a database only when you truly
      need it — accounts, orders, content created by visitor actions.
  - q: "How much git do I have to know?"
    a: >-
      Three things to start: a commit is a saved version of the whole
      project, a branch is a parallel line of history, and rolling back means
      moving to an earlier point without losing history. One rule: commit in
      small steps with messages that make sense a month later — the worst
      that can then happen is losing the work since the last commit.
  - q: "Can AI suggest an available domain for me?"
    a: >-
      Ideas and language instinct yes, availability no — a model has no
      reliable picture of which domains are free, and if you push it, it
      makes one up. Check availability with a registrar or the registry, run
      the name through a search engine and the trademark registry. And let
      a connected domain propagate for a day before announcing it — DNS can't
      be rushed.
  - q: "Where do keys and passwords go?"
    a: >-
      Never into the repository — even in a private one, a key stays in the
      history forever and travels through backups and clones. They belong in
      Vercel's environment variables, and locally in a file excluded by
      .gitignore. Set up .gitignore before you write the first key, and have
      the agent check the whole history while it's still short.
  - q: "What may the agent do on its own, and what never?"
    a: >-
      Without asking: read, search, build, and edit files — the worst outcome
      gets discarded with one command. With approval: installing dependencies
      and bulk edits. Never without an explicit instruction: deleting files,
      rewriting git history, and deploying to production — a push to the main
      branch is a release, and a human approves releases. Write the rules
      into a file the agent reads on every start.
---
