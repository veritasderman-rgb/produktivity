...
excerpt: "A guide with prompts: an executive summary with the conclusion first, charts readable in ten seconds, prep for tough questions, and a monthly routine."
category: "ai"
platform: "vsude"
audience: ["manazer"]
keys: []
...
updated: "2026-08-12"
tldr: >-
  A report to leadership isn't a data overview — it's the basis for a decision:
  data is evidence, the content is the conclusion and the thing you want from
  leadership. The guide runs from numbers computed by a script, through an
  executive summary with the conclusion on the first line (BLUF), a four-block
  structure and charts readable in ten seconds, to a simulated CFO who tears the
  report apart a day before the meeting — and finally turns the whole prep into
  a monthly routine.
faq:
  - q: "What's the difference between a report and a status update?"
    a: >-
      A report is the basis for a decision: it contains a conclusion, a cause,
      what you're doing about it, and what you want from leadership — money,
      a hire, a priority, or a green light. A status update just describes what's
      in the data. If you can't tell from the document what decision should be
      made and by whom, it isn't finished.
  - q: "Can I have the model compute the report numbers right in the chat?"
    a: >-
      No — the model doesn't calculate, it writes, and it only sees a slice of
      a long table. A script does the aggregation, the model does the
      interpretation: export the data to CSV, show the model the header and
      a few rows, and have it write a script that produces the same numbers
      every time.
  - q: "Where does the main conclusion belong in the report?"
    a: >-
      On the first line — the BLUF principle, “bottom line up front”. The
      executive summary is three to five sentences: the state, the cause, what
      you're doing about it, and what you need from leadership. If the reader
      has to get to the last slide to find out what it's about, they usually
      won't.
  - q: "How do I catch myself over-interpreting the data?"
    a: >-
      Have the model split your claims into supported, hypothesis, and
      unsupported. “The decline was caused by two colleagues leaving” is a
      hypothesis until the data rules out other explanations — you don't delete
      it, you reword it: “the timing lines up with two colleagues leaving; we
      haven't found another explanation”. That difference costs five words and
      carries the report's credibility.
  - q: "What is the simulated CFO for?"
    a: >-
      Ten minutes before sending, you have the report torn apart by the model
      playing a direct, adversarial CFO — so you hear nine out of ten
      uncomfortable questions a day early, not in the meeting. Verify the
      findings against the text, though: the model sometimes criticizes
      something that's in the report and it simply overlooked.
  - q: "Can the scheduled routine send the report itself?"
    a: >-
      No — the task prompt includes the sentence “never send anything”, and it
      belongs there every time. Sending is a human decision, because the
      report's content is your judgment of the situation, not a tool's output.
      The routine prepares a draft with the numbers; you write the conclusion,
      the cause, and the ask.
---
