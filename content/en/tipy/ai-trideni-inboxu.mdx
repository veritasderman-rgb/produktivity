...
excerpt: "A guide with prompts: action-based categories, an escalation rule so the boss's mail never vanishes, and a weekly audit. Move and label — never delete."
category: "ai"
platform: "vsude"
...
updated: "2026-08-12"
tldr: >-
  The goal of triage isn't inbox zero — it's an inbox that contains only work for a human: four action-based categories (reply today, delegate, read, archive), hard filters for notifications, and AI only where content decides. The most important pieces are a “never filter” escalation list for your boss and key clients, and a weekly audit of false positives. Triage may move and label — never delete.
faq:
  - q: >-
      Why not sort by projects and topics?
    a: >-
      Because when you open your inbox, you're not deciding what a message is about — you're deciding what to do with it. A “Project Alfa” label doesn't tell you whether something is waiting on you today. Four action-based categories are enough — reply today, delegate, read, archive — named with verbs, not nouns.
  - q: >-
      Do I even need AI for triage?
    a: >-
      Mostly no — a rule is free, instant, and never wrong. Notifications, newsletters, and confirmations get caught by ordinary filters, typically sixty to eighty percent of the volume. AI pays off only where content decides: no rule can tell “please send a quote” apart from “just letting you know the deadline changed.”
  - q: >-
      What if triage hides an important email from my boss?
    a: >-
      That's exactly what the escalation list is for: your manager, key clients, and contract-related addresses go on a hard “never filter” list, phrased as a prohibition — the routine may never move or label those messages, even when they look like a mailing. A mistake in that direction costs trust, while an extra newsletter in the inbox costs a second.
  - q: >-
      May the routine delete obvious spam?
    a: >-
      No — triage may move and label, never delete, and this boundary has no exception. A misfiled message can be found in a folder; a deleted one can't. Don't grant the automation delete permission even at the technical level, if that's configurable.
  - q: >-
      What should the routine do when it's not sure?
    a: >-
      Not guess: an uncertain message goes to a “Review triage” folder with a note about which categories it hesitated between. More flagged uncertainty beats a silent mistake — an empty uncertainty folder means the model is guessing instead of admitting it doesn't know. And before going live, run it for a week in suggest-only mode.
  - q: >-
      How do I know the triage isn't degrading?
    a: >-
      With a weekly audit: compare what the routine filed with what you actually did with the messages — false positives, needlessly kept messages, error patterns, and above all any touch of the escalation list. It's done when the audit finds zero false positives three weeks in a row; then a monthly check is enough.
---
