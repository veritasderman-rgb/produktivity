...
excerpt: "A complete guide with prompts: what MCP is and why it's the USB-C of AI, connecting mail, calendar, or Notion, and least-privilege permission setup."
category: "ai"
platform: "vsude"
audience: ["vyvojar"]
keys: []
...
updated: "2026-08-12"
tldr: >-
  MCP is an open standard that works like USB-C for AI: plug in a connector once and
  the assistant sees straight into your mail, calendar, Notion, or Drive — no more
  shuttling data back and forth. You proceed one connector at a time, read-only for
  the first week, with permissions set by the least-privilege principle. A connector
  is permission to read and prepare, not to send or pay — anything with an
  outward-facing consequence is approved by a human.
faq:
  - q: "Does a connector mean AI has access to all my data?"
    a: >-
      No. The model can't reach wherever it wants — only what the connector offers
      and what you've allowed, and it runs under your account and your permissions,
      so it sees no more than you do. A connector also requests data only at the
      moment it needs it for an answer; it doesn't bulk-download your mail in
      advance.
  - q: "How should I set connector permissions?"
    a: >-
      By the least-privilege principle: read instead of write whenever it's enough,
      a narrower scope instead of a wider one (one folder, not the whole drive),
      work account for work things, and no permission granted “just in case.” The
      proven sequence: one connector to start, a week of read-only, then write —
      and the next connector only after that.
  - q: "Can the assistant be influenced by text it reads in mail or documents?"
    a: >-
      Yes — that's the risk almost nobody talks about. A message with a hidden
      instruction can influence the assistant, because it doesn't distinguish
      between your brief and the content it reads as reliably as it should. The
      defense: outward-facing actions are approved by a human. With read-only
      rights the worst outcome is a strange answer; with sending rights it's a
      different story.
  - q: "May an unattended routine send things on its own?"
    a: >-
      No — a routine ends with a brief or a draft, not an action. It prepares a
      summary, a draft reply, or an alert; sending, confirming, and paying are done
      by a human. And explicitly allow it to write “nothing this week”: a routine
      tasked with finding something otherwise tends to find something every time,
      even in a week when nothing happened.
  - q: "When is a custom MCP server worth it for a company?"
    a: >-
      When the data lives in a system with no ready-made connector and people
      regularly copy from it, when many people ask the same questions repeatedly,
      when the answer can be retrieved reliably, when it saves more than a few
      minutes a day — and when there's an owner who will maintain the server as the
      underlying system changes. The hardest part isn't programming but writing down
      what it should be able to answer.
  - q: "How do I know an answer from connectors isn't made up?"
    a: >-
      Demand a source for every claim — a link or a document name. Without that you
      can't tell what the assistant actually found from what it filled in. When an
      answer seems off, have it describe its process: which connectors it used, what
      it searched for, and which claims come from data versus its own conclusions.
---
