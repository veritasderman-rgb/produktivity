...
updated: "2026-08-12"
tldr: >-
  The data for a budget already exists — your bank has been keeping it for years,
  line by line. In one evening, a twelve-month export becomes a complete picture:
  anonymization, categorization with rules, monthly cash flow, and a list of
  forgotten subscriptions. To make it stick, you need a ten-minute monthly routine
  with the same saved prompt. A statement is one of the most sensitive things you
  own — and canceling payments or moving money is done by you, in your own online
  banking.
faq:
  - q: "Why download twelve months instead of three?"
    a: >-
      The yearly window is the only one where you see things that happen once a
      year: insurance, road tolls, taxes, vacation, holiday shopping. Analyzing a
      quarter produces a distorted picture of a “normal month” that doesn't
      actually exist — and then you wonder why the budget doesn't add up.
  - q: "What should I delete from the statement before uploading it?"
    a: >-
      Delete account numbers, payment symbols, addresses, and full names of people
      entirely; keep the date, amount, payment type, and a rough description of the
      expense. Replace sensitive names (employer, doctor, a person you send money
      to) with a generic label. Work only in a paid account with contractual data
      protection and remove the file from the conversation afterward — anonymization
      is best done by a script on your own computer.
  - q: "What's the difference between cash flow and the account balance?"
    a: >-
      Cash flow is the difference between what came in and went out that month; the
      balance is the sum of the account's entire history. You can have a hundred
      thousand in the account and still run negative cash flow five months in a
      row — the account looks healthy until the reserve runs out. Confusing the
      two is the most expensive mistake in personal finance.
  - q: "What if the model isn't sure about an unclear payment?"
    a: >-
      It must not guess — without the explicit instruction “don't guess, put it in
      unassigned,” unclear payments get quietly distributed by vibe and you end up
      with precise-looking numbers that don't add up. You fill in the unassigned
      list by hand in a few minutes; that's cheaper than distrusting your own
      budget.
  - q: "Can AI cancel an unused subscription for me?"
    a: >-
      No — canceling payments is done by a human, in their banking or directly with
      the service. AI prepares a decision table with questions, but cancellations
      surprisingly often reveal something hanging on the service: a shared family
      account, a domain, photo backups. And with annual payments it usually makes
      sense to let the service run out and just turn off renewal.
  - q: "Why doesn't a one-off analysis last?"
    a: >-
      Because it's a snapshot, not a gauge — decisions drift back to old habits
      without measurement. The key is a ten-minute routine on the first weekend of
      the month: download the statement, run the saved prompt with your mapping
      rules, add one row, and glance at three numbers. Only comparison with
      previous months reveals that delivery spending has grown three months in a
      row.
---
